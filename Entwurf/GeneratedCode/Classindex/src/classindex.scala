...
object ClassIndex extends App {
  ...
  val serverPacks = packages.filter { _ contains "studyplan.server" }
  val clientPacks = packages.filter { _ contains "studyplan.client" }
...
//  println
//
//  packages map {_.stripPrefix("edu.kit.informatik.studyplan.")} diff List("client.model", "server.model") foreach { p => {
...
//  }}
  ...
  println("\t% auto-generated by classindex.scala")
  packages map {_.stripPrefix("edu.kit.informatik.studyplan.")} diff List("client.model", "org.hibernate", "org.backbone") foreach { p => {
