...
Im Folgenden werden die zur REST-Kommunikation nötigen JSON-Datenklassen mithilfe der in Kapitel \ref{subsec:rest-notation} beschriebenen Notation spezifiziert. 
...
\begin{lstlisting}[language=json]
(*\lbljsonobj{Kategorie}*) = {
	"id": (*\jsonatom{Kategorie-ID}*),
	"name": (*\jsonatom{Kategorie-Name}*)
}
\end{lstlisting}
...
(*\lbljsonobj{ModuleResult}*) = {
	"module": (*\jsonpart{Modul}{id, name, categories, creditpoints, lecturer, description, constraints}*)
}	
\end{json}
...
(*\lbljsonobj{SubjectsResult}*) = {
	"subjects": (*\jsonlist{\jsonobj{Kategorie}}*)
}
\end{json}